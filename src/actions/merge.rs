...
#[instrument(skip(merge_output_name, selected_fleets, main_window))]
fn merge_fleets(
    merge_output_name: String,
    selected_fleets: &[FleetData],
    ...
    if !selected_fleets
        ...
        .filter_map(|fleet| {
            ...
            debug!("Pulling ships from fleet at '{}'", fleet.path);
            Reader::new(EventReader::new(&file), &mut ships, "Ships", "Ship").run_until_complete();
            ...
            debug!("Pulling missile types from fleet at '{}'", fleet.path);
            Reader::new(
    ...
    trace!("Initialising fleet writer");
...
impl<W: Write, R: Read> Writer<W, R> {
    ...
    pub fn run_until_complete(&mut self) {
        trace!("Copying initial fields from primary fleet");
        while !(self.is_done() || self.is_error()) {
            self.tick();
        }
    }
