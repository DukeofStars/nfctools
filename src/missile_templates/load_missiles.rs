...
pub fn on_reload_missiles_handler(
    ...
    move || {
        ...
        let _ = wrap_errorable_function_m(&window, || {
            ...
            let missiles = load_missiles(&missiles_dir).map_err(|err| my_error!("Failed to load fleets", err))?;
...
pub fn load_missiles(path: impl AsRef<Path>) -> Result<Vec<MissileData>, Error> {
    ...
    for child in children {
        ...
        if file_type.is_file() {
            ...
            let missile = match read_missile(&path) {
                Ok(missile) => missile,
                Err(err) => {
                    warn!("Skipping invalid missile '{}': {}", path.display(), err);
                    continue;
                }
            };

            output.push(MissileData {
                template_name: missile.associated_template_name.unwrap_or_default().to_shared_string(),
                ...
                cost: missile
                    .cost
                    .parse::<i32>()
                    .map_err(|err| my_error!("Invalid missile file", format!("Failed to parse cost: {}", err)))?,
                selected: false,
            });
        }
    }
